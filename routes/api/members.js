const express = require('express');
const router = express.Router();
const members = require('../../Members');
const uuid = require('uuid');

//Get All Members
router.get('/', (req, res) => {
    res.json(members);
})

//Get Single Member
router.get('/:id', (req, res) => {
    const found = members.some(member => member.id.toString() === req.params.id);
    if(found) {
        res.json(members.filter(member => member.id.toString() === req.params.id))
    } else {
        //bad request HTTP status code
        res.status(400).json({ msg: `No member with id of ${req.params.id}`});
    }
})

//Create Member
router.post('/', (req, res) => {
    const newMember = {
        id: uuid.v4(),
        name: req.body.name,
        email: req.body.email,
        status: 'active'
    }

    if(!newMember.name || !newMember.email) {
        return res.status(400).json({msg: 'Please include a name and email' });
    }

    members.push(newMember);
    // res.json(members);
    res.redirect('/');
});

// Update Member
router.put('/:id', (req, res) => {
    const found = members.some(member => member.id.toString() === req.params.id);
    if(found) {
        const updMember = req.body;
        members.forEach(member => {
            if(member.id.toString() === req.params.id) {
                member.name = updMember.name ? updMember.name : member.name;
                member.email = updMember.email ? updMember.email : member.name;

                res.json({ msg: "Member updated", member})
            }
        })
    } else {
        //bad request HTTP status code
        res.status(400).json({ msg: `No member with id of ${req.params.id}`});
    }
})

//Delete Member
router.delete('/:id', (req, res) => {
    const found = members.some(member => member.id.toString() === req.params.id);
    if(found) {
        res.json({ msg: "Member deleted", members: members.filter(member => member.id.toString() !== req.params.id)})
    } else {
        //bad request HTTP status code
        res.status(400).json({ msg: `No member with id of ${req.params.id}`});
    }
})

module.exports = router;